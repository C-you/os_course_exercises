...
CPL: 当前进程的特权级
DPL: 描述符特权级，规定访问该段的权限级别
RPL: 请求特权级，表示进程对段访问的特权级
段描述符中存在DPL字段，段选择子中存在RPL字段
...
为了防止用户修改中断服务程序相应的代码和数据
3. x86中trap类型的中断门与interrupt类型的中断门有啥设置上的差别？如果在设置中断门上不做区分，会有什么可能的后果?
当调用Interrupt gate时，Interrupt会被CPU自动禁止，而Trap则不会。
### 4.3 练习四和五 ucore内核映像加载和函数调用栈分析

1. ucore中，在kdebug.c文件中用到的函数`read_ebp`是内联的，而函数`read_eip`不是内联的。为什么要设计成这样？
内联的函数可使读取的结果正好是print_stackframe()函数的ebp地址。而不是内联的，是在该函数中读取的是返回地址的值，对应的是print_stackframe()函数中下一条语句的地址。
### 4.4 练习六 完善中断初始化和处理
