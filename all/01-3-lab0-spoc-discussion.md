...
链表节点list_entry不包括传统的data数据域，而是在具体的数据结构中包含链表节点。 链表操作函数有初始化、插入、删除、访问链表节点所在的宿主数据结构等。
...
1. 请在ucore中找一段你认为难度适当的AT&T格式X86汇编代码，尝试解释其含义。
   xorw %ax, %ax                                   将ax置零
    movw %ax, %ds                             将Data Segment置零
    movw %ax, %es                             将Extra Segment置零
    movw %ax, %ss    		         将Stack Segment置零
2. (option)请在rcore中找一段你认为难度适当的RV汇编代码，尝试解释其含义。

#### 练习二

宏定义和引用在内核代码中很常用。请枚举ucore或rcore中宏定义的用途，并举例描述其含义。
在第二章2.6里，这个宏通过page结构的链表节点成员list_entry来访问page > > mmu.h中的宏SETGATE用来设置中断描述符
#### reference
